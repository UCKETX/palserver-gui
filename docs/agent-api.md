...
| `PUT /api/instances/:id/discord-bot` | 型別 `DiscordBotSettings` 的部分欄位 + `token?`(enabled/adminUserIds/notifyChannels/statusChannelIds/language);`notifyChannels` 每項為 `{ channelId, events }` | 更新後 `DiscordBotStatus` | |
