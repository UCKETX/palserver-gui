 ...
 *
 * 設計:程式碼裡的字串一律寫繁中原文,t() 拿原文當 key 查字典;各語言字典是
...
function detectLang(): Lang {
  try {
    ...
    if (stored === "zh") return "zh-TW"; // migrate the pre-zh-CN locale value
    if (stored === "zh-TW" || stored === "zh-CN" || stored === "en" || stored === "ja") {
      return stored;
    }
  } catch {
    /* ignore */
  ...
  if (nav === "zh-tw" || nav === "zh-hk" || nav === "zh-mo" || nav.startsWith("zh-hant")) {
    return "zh-TW";
  }
...
export function t(source: string, params?: Record<string, string | number>): string {
  let out = (lang !== "zh-TW" && dicts[lang]?.[source]) || source;
  if (params) {
